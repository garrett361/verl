...
"""
Preprocess the Skywork dataset to parquet format
"""
...
if __name__ == '__main__':
    ...
    # do a conversion for math data to standardize 
    # reward model to be a dictionary
    for item in math_data_list:
        item['reward_model']['ground_truth'] = {
            'math-verify': item['reward_model']['ground_truth'][0]
        }
    ...
    pd.DataFrame(math_data_list).to_parquet(
        os.path.join(local_dir, 'train_7b_math.parquet')
    )
    pd.DataFrame(code_data_list).to_parquet(
        os.path.join(local_dir, 'train_7b_code.parquet')
    )
